(() => {
  ...
  function choosePrimary(hits, q) {
    ...
    const priority = ["culture", "city", "ecology", "philosophy", "fixedQuote", "digital", "material"];
    return hits.find(rule => priority.includes(rule.key)) || hits[0] || RULES.find(rule => rule.key === "material");
  }
  function reasons(q, primary) {
    const text = qText(q);
    const words = ["AI", "人工智能", "智慧监管", "平台", "标准化", "追溯", "首发经济", "15分钟", "养老", "高校", "企业", "枫桥", "信访", "扫黑除恶", "文创", "盲盒", "山水林田湖草沙", "转危为机", "化险为夷"].filter(w => text.includes(w));
    const out = [`已自动选主卡：${primary.label}`];
    if (words.length) out.push(`命中词：${words.slice(0, 8).join("、")}`);
    if (/主要意在|主要表明|体现了|以上做法/.test(text)) out.push("问法是材料映射题，材料题总法作为辅助规则。");
    return out;
  }
  function autoLink(q, allCards) {
    ...
    return {
      ...
      cardIds, primaryCardId, autoMatched: true, autoSelected: true,
  ...
  function render() {
    ...
    visibleBoxes().forEach(box => {
      ...
      const link = existing(store, q) || upsert(store, autoLink(q, allCards));
