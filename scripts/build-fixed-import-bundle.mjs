...
function validateLocalPaths(config) {
  const paths = {
  ...
  if (fs.existsSync(paths.historicalTruthFile) && !fs.statSync(paths.historicalTruthFile).isFile()) {
    fail("historicalTruthFile 不是文件。", [`historicalTruthFile: ${paths.historicalTruthFile}`]);
  }

  return paths;
}

const localPaths = validateLocalPaths(readLocalPaths());
const PAPER_DIR = localPaths.paperDir;
const ANSWER_DIR = localPaths.answerDir;
const HISTORICAL_TRUTH_FILE = localPaths.historicalTruthFile;
const OUT_JS = localPaths.outJs;
const OUT_REPORT = localPaths.outReport;
